...
pub trait ActiveEventLoopExtWayland {
    ...
    /// Create an `xdg_popup` whose parent is another Wayland window.
    fn create_popup_wayland(
        &self,
...
impl ActiveEventLoopExtWayland for dyn CoreActiveEventLoop + '_ {
    ...
    fn create_popup_wayland(
        ...
        let event_loop = self.cast_ref::<ActiveEventLoop>().ok_or_else(|| {
            RequestError::from(NotSupportedError::new("event loop is not using Wayland"))
        })?;
        Ok(Box::new(Window::new_popup(event_loop, parent, attributes)?))
    }
}
...
/// Anchor used to place an xdg popup relative to its anchor rectangle.
...
/// Attributes for a compositor-positioned Wayland xdg popup.
#[derive(Debug, Clone)]
pub struct PopupAttributesWayland {
