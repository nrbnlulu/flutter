...
use sctk::compositor::{CompositorState, Region, Surface, SurfaceData};
...
use crate::{PopupAnchor, PopupAttributesWayland, WindowAttributesWayland, output};
...
pub struct Window {
    ...
    /// Surface exposed to renderers and input routing. For popups this is
    /// distinct from the private toplevel retained by `window_state`.
    surface: WlSurface,

    /// Keeps an xdg_popup role alive when this is a popup window.
    popup: Option<Popup>,
...
impl Window {
    pub(crate) fn new_popup(
        ...
        parent: &dyn CoreWindow,
        attributes: PopupAttributesWayland,
    ) -> Result<Self, RequestError> {
        let parent = parent.cast_ref::<Window>().ok_or_else(|| {
            RequestError::from(NotSupportedError::new("popup parent is not a Wayland window"))
        })?;
        ...
        let size = Size::Logical(attributes.size.cast::<f64>());

        // WindowState currently owns toplevel-specific bookkeeping. Keep a private,
        // uncommitted toplevel for that bookkeeping while exposing only the real
        // popup surface to rendering and input routing.
        let state_surface = state.compositor_state.create_surface(&queue_handle);
        let state_window = state.xdg_shell.create_window(
            state_surface,
        ...
        popup_state.set_decorate(false);
        popup_state.set_transparent(true);

        let positioner = XdgPositioner::new(&state.xdg_shell).map_err(|err| os_error!(err))?;
        positioner.set_size(attributes.size.width.max(1) as i32, attributes.size.height.max(1) as i32);
        let (x, y, width, height) = attributes.anchor_rect;
        positioner.set_anchor_rect(x, y, width.max(1), height.max(1));
        positioner.set_anchor(popup_anchor(attributes.anchor));
        positioner.set_gravity(popup_gravity(attributes.gravity));
        positioner.set_offset(attributes.offset.0, attributes.offset.1);
        positioner.set_constraint_adjustment(
            ConstraintAdjustment::from_bits_truncate(attributes.constraint_adjustment),
        );
        if attributes.reactive {
            positioner.set_reactive();
        }

        let popup_surface = Surface::new(&*state.compositor_state, &queue_handle)
        ...
        .map_err(|err| os_error!(err))?;
        if attributes.grab {
            if let Some((seat, serial)) = state.seats.values().find_map(|seat| seat.popup_grab()) {
        ...
        let window_state = Arc::new(Mutex::new(popup_state));
    ...
    fn xdg_surface(&self) -> &sctk::reexports::protocols::xdg::shell::client::xdg_surface::XdgSurface {
        self.popup
            .as_ref()
            .map(Popup::xdg_surface)
            .unwrap_or_else(|| self.window.xdg_surface())
    }
}

fn popup_anchor(anchor: PopupAnchor) -> Anchor {
    match anchor {
...
fn popup_gravity(anchor: PopupAnchor) -> Gravity {
    match anchor {
        PopupAnchor::None => Gravity::None,
        PopupAnchor::Top => Gravity::Top,
        PopupAnchor::Bottom => Gravity::Bottom,
        PopupAnchor::Left => Gravity::Left,
        PopupAnchor::Right => Gravity::Right,
        PopupAnchor::TopLeft => Gravity::TopLeft,
        PopupAnchor::BottomLeft => Gravity::BottomLeft,
        PopupAnchor::TopRight => Gravity::TopRight,
        PopupAnchor::BottomRight => Gravity::BottomRight,
    }
}
...
impl CoreWindow for Window {
    ...
    fn pre_present_notify(&self) {
        self.window_state.lock().unwrap().request_frame_callback(&self.surface);
    }
