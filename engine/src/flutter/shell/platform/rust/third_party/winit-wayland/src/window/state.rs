...
pub struct WindowState {
    ...
    /// Whether an xdg_popup received its initial configure.
...
impl WindowState {
    ...
    pub fn request_frame_callback(&mut self, surface: &WlSurface) {
    ...
    pub fn is_configured(&self) -> bool {
        self.last_configure.is_some() || self.popup_configured
    }

    pub(crate) fn configure_popup(&mut self, width: i32, height: i32) -> bool {
        let size = LogicalSize::new(width.max(1) as u32, height.max(1) as u32);
        let changed = self.size != size;
        self.size = size;
        self.stateless_size = size;
